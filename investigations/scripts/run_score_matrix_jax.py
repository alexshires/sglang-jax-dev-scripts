#!/usr/bin/env python3
"""Run client chunk-size matrix benchmarks against a JAX /v1/score endpoint."""
...
def detect_hardware() -> Dict[str, Any]:
    ...
    tpu_env = {k: os.getenv(k) for k in env_keys if os.getenv(k)}
...
def write_markdown_summary(path: Path, payload: Dict[str, Any]) -> None:
    lines: List[str] = []
    lines.append("# JAX Score Matrix Results")
    lines.append("")
