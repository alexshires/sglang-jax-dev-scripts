...
class TokenizerManager:
    ...
    async def score_request(
        self,
        query: list[list[int]] | list[str] | list[int] | str | None = None,
        items: list[list[[list[int]]]] | list[list[str]] | str | list[str] |  None = None,
        label_token_ids: list[list[int]] | list[int] | None = None,
        ...
        def _convert_logprobs_multiple_queries(logprobs_data: list, label_token_ids: list[int]) -> list[float]:
            logprobs = {}
            for logprob, token_id, _ in logprobs_data:
                if token_id in label_token_ids:
                    logprobs[token_id] = logprob
        ...
        async def _score_multi_item_tokenized(
            ...
            for start in range(0, item_count, chunk_size):
                ...
                print("Results calculated",flush=True,file=sys.stderr)
                print(result,flush=True,file=sys.stderr)
                print("input logprobs",flush=True,file=sys.stderr)
                print(input_logprobs,flush=True,file=sys.stderr)
        ...
        async def _score_multi_item_tokenized_multiple_queries(
            query_tokens_multiple: list[list[int]], item_tokens_multiple: list[list[int]]
        ) -> list[list[float]]:
            # 1. Setup chunk size parameters
            chunk_size = int(self.server_args.multi_item_scoring_chunk_size)
            
            combined_input = []
            # This will store (original_query_index, items_in_this_chunk) to help reassemble
            chunk_metadata = [] 
            
            print(f"In multiple query scoring with chunk_size={chunk_size}", flush=True, file=sys.stderr)

            # 2. Build the chunked sequences
            for i in range(len(query_tokens_multiple)):
                query_tokens = query_tokens_multiple[i]
                item_group = item_tokens_multiple[i]
                item_count = len(item_group)

                # Determine effective chunk size for this specific query group
                current_chunk_size = chunk_size if chunk_size > 0 else item_count
                
                # Split item_group into smaller chunks
                for j in range(0, item_count, current_chunk_size):
                    item_chunk = item_group[j : j + current_chunk_size]
                    
                    # Create the sequence: [Q, D, I1, D, I2, ..., D]
                    current_sequence = list(query_tokens)
                    for item_tokens in item_chunk:
                        current_sequence.append(delimiter_token_id)
                        current_sequence.extend(item_tokens)
                    current_sequence.append(delimiter_token_id)
                    
                    combined_input.append(current_sequence)
                    # Track which query this chunk belongs to and how many items are in it
                    chunk_metadata.append((i, len(item_chunk)))

            # 3. Padding and Request Construction
            max_len = max(len(s) for s in combined_input)
            pad_id = self.tokenizer.pad_token_id if self.tokenizer.pad_token_id is not None else 0
            padded_input = [s + [pad_id] * (max_len - len(s)) for s in combined_input]
            
            # Note: label_token_ids must be mapped to match the NEW batch size (combined_input)
            # If label_token_ids[i] is specific to the query, we replicate it for each chunk
            mapped_label_token_ids = [label_token_ids[m[0]] for m in chunk_metadata]

            batch_request = GenerateReqInput(
                input_ids=padded_input,
                return_logprob=True,
                logprob_start_len=0,
                token_ids_logprob=mapped_label_token_ids,
                stream=False,
                sampling_params={"max_new_tokens": 0},
                is_multi_item_scoring=True,
                multi_item_scoring_delimiter=delimiter_token_id,
            ...
            # 5. Reassemble Scores
            # Prepare an empty list of lists for each original query
            all_scores: list[list[float]] = [[] for _ in range(len(query_tokens_multiple))]

            for idx, result in enumerate(results):
                query_idx, num_items_in_chunk = chunk_metadata[idx]
                input_logprobs = result["meta_info"].get("input_token_ids_logprobs", [])
                
                if not input_logprobs:
                    raise RuntimeError(f"Logprobs missing for chunk {idx}")

                # Calculate scores for items in THIS chunk
                chunk_scores = [
                    _convert_logprobs_multiple_queries(
                        input_logprobs[item_idx + 1], 
                        label_token_ids[query_idx]
                    ) for item_idx in range(num_items_in_chunk)
                ]
                
                # Append these scores to the correct original query index
                all_scores[query_idx].extend(chunk_scores)
        ...
        if use_multi_item_scoring:
            if isinstance(query, str) and isinstance(items, list) and (
                ...
                isinstance(query, list) and isinstance(query[0],int)
                ...
                print("Multiple queries detected",flush=True,file=sys.stderr)
                query_tokens_multiple = []
                item_tokens_multiple = []
                if self.tokenizer is None:
                    raise ValueError("Tokenizer is required for text multi-item scoring.")
                print("printing queries",flush=True,file=sys.stderr)
                for i in range(0,len(query)):
                    print(query[i],flush=True,file=sys.stderr)
                    query_tokens = self.tokenizer.encode(query[i], add_special_tokens=False)
                    item_tokens = [self.tokenizer.encode(item, add_special_tokens=False) for item in items[i]]
                    query_tokens_multiple.append(query_tokens)
                    item_tokens_multiple.append(item_tokens)
                return await _score_multi_item_tokenized_multiple_queries(query_tokens_multiple, item_tokens_multiple)
            elif (
                isinstance(query, list) and isinstance(query[0], list)
                and isinstance(items, list)
                and items
                and isinstance(items[0], list)
            ):
                return await _score_multi_item_tokenized_multiple_queries(query, items)
            else:
